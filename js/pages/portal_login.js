...
/**
 * Nurses and doctors sign in with an email + password issued by the team,
 * instead of the WhatsApp magic link patients use. The password itself is
 * never stored in our tables — it lives entirely in Supabase Auth
 * (auth.users), and this just verifies it and maps the resulting auth user
 * onto the nurses/doctors row via auth_user_id (see sql/12_staff_email_login.sql).
 *
 * Same "one generic answer" discipline as every other login path here: wrong
 * password, unknown email, and a staff row that exists in Auth but was never
 * linked (or was since deactivated) all come back as invalid_credentials —
 * an attacker learns nothing about which one it was.
 */
async function passwordLogin(role: PortalRole, rawEmail: string, rawPassword: string): Promise<Response> {
  ...
  // Verify the credential against Supabase Auth directly. This just needs a
  // valid project API key (the service key qualifies) plus a correct
  // email/password — it returns THAT user's identity, not an admin session.
  const { data: authData, error: authErr } = await db.auth.signInWithPassword({ email, password });
  if (authErr || !authData?.user) {
    // Logged (never shown to the client — the client always gets the same
    // generic invalid_credentials either way) so we can actually tell wrong
    // password apart from an unconfirmed email or an API-key mismatch
    // instead of guessing from outside.
  ...
  const { error: sessErr } = await db.from('portal_sessions').insert({
    ...
    role,
    person_id: p.id,
    phone: normPhone(String(p.phone ?? '')),
    expires_at: expiresAt,
    last_seen_at: new Date().toISOString(),
  });
  if (sessErr) {
